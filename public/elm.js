...
var $author$project$Main$NewRectangle = function (a) {
	return {$: 'NewRectangle', a: a};
};
...
var $author$project$MathGen$Rectangle = F2(
	function (h, w) {
		return {h: h, w: w};
	});
...
var $author$project$MathGen$getQuestion = F2(
	function (max, scores) {
		var weighted = A2(
...
var $author$project$MathGen$initNumberScores = {eights: 10, fives: 10, fours: 10, nines: 10, ones: 10, sevens: 10, sixes: 10, tens: 10, threes: 10, twos: 10};
		...
		switch (msg.$) {
			...
			case 'NewRectangle':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{countDown: 10, rectangle: rectangle}),
					$elm$core$Platform$Cmd$none);
			case 'KeyMsg':
				var km = msg.a;
				var newKeys = A2($ohanhi$keyboard$Keyboard$update, km, _List_Nil);
				var getNum = function (k) {
					if (k.$ === 'Character') {
						var c = k.a;
						return $elm$core$String$toInt(c);
					} else {
						return $elm$core$Maybe$Nothing;
					}
				};
				var newNums = A2($elm$core$List$filterMap, getNum, newKeys);
				var enterPressed = A2($elm$core$List$member, $ohanhi$keyboard$Keyboard$Enter, newKeys);
				var backspacePressed = A2($elm$core$List$member, $ohanhi$keyboard$Keyboard$Backspace, newKeys);
				var handleBackspace = function (inputs) {
					return backspacePressed ? $elm$core$List$reverse(
						A2(
							$elm$core$List$drop,
							1,
							$elm$core$List$reverse(inputs))) : inputs;
				};
				var numInput = handleBackspace(
					function (l) {
						return A2(
							$elm$core$List$drop,
							$elm$core$List$length(l) - 3,
							l);
					}(
						_Utils_ap(model.numInput, newNums)));
				var model_ = _Utils_update(
					model,
					{numInput: numInput});
				return enterPressed ? $author$project$Main$checkAnswer(model_) : _Utils_Tuple2(model_, $elm$core$Platform$Cmd$none);
			default:
				var t = msg.a;
				return (model.countDown < 2) ? $author$project$Main$checkAnswer(
					_Utils_update(
						model,
						{countDown: model.countDown - 1})) : _Utils_Tuple2(
					_Utils_update(
						model,
...
var $author$project$Main$viewCountDown = F2(
...
var $author$project$Main$viewFullscreenRect = F3(
	function (window, color, renderable) {
		return _Utils_ap(
			renderable,
			_List_fromArray(
				[
...
var $author$project$Main$viewRectangle = F4(
	function (rectangle, window, color, renderables) {
		...
		var basis = A2($elm$core$Basics$min, window.width / 20, window.height / 20);
		var height = basis * rectangle.h;
		var width = basis * rectangle.w;
		var _v0 = _Utils_Tuple2((window.width / 2) - (width / 2), (window.height / 2) - (height / 2));
		var x = _v0.a;
		var y = _v0.b;
		var textLeftPos = _Utils_Tuple2(x - 20, window.height / 2);
		var textRightPos = _Utils_Tuple2(window.width / 2, (y + height) + 20);
		return _Utils_ap(
			renderables,
			_List_fromArray(
				[
					...
					textSettings,
					textLeftPos,
					$elm$core$String$fromInt(rectangle.h)),
					A3(
					$author$project$Main$text,
					textSettings,
					textRightPos,
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_Utils_Tuple2(
					$elm$core$Basics$floor(model.window.width),
					$elm$core$Basics$floor(model.window.height)),
				_List_Nil,
